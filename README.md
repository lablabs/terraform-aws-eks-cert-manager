...
| <a name="module_cluster_issuer"></a> [cluster\_issuer](#module\_cluster\_issuer) | git::https://github.com/lablabs/terraform-aws-eks-universal-addon.git//modules/addon | v0.0.18 |
## Resources
...
| <a name="input_cluster_issuer_argo_apiversion"></a> [cluster\_issuer\_argo\_apiversion](#input\_cluster\_issuer\_argo\_apiversion) | ArgoCD Application apiVersion. Defaults to `argoproj.io/v1alpha1`. | `string` |
| <a name="input_cluster_issuer_argo_destination_server"></a> [cluster\_issuer\_argo\_destination\_server](#input\_cluster\_issuer\_argo\_destination\_server) | Destination server for ArgoCD Application. Defaults to `https://kubernetes.default.svc`. | `string` |
| <a name="input_cluster_issuer_argo_enabled"></a> [cluster\_issuer\_argo\_enabled](#input\_cluster\_issuer\_argo\_enabled) | If set to `true`, the module will be deployed as ArgoCD Application, otherwise it will be deployed as a Helm release. Defaults to `false`. | `bool` |
| <a name="input_cluster_issuer_argo_helm_enabled"></a> [cluster\_issuer\_argo\_helm\_enabled](#input\_cluster\_issuer\_argo\_helm\_enabled) | If set to `true`, the ArgoCD Application manifest will be deployed using Kubernetes provider as a Helm release. Otherwise it'll be deployed as a Kubernetes manifest. See README for more info. Defaults to `false`. | `bool` |
| <a name="input_cluster_issuer_argo_helm_values"></a> [cluster\_issuer\_argo\_helm\_values](#input\_cluster\_issuer\_argo\_helm\_values) | Value overrides to use when deploying ArgoCD Application object with Helm. Defaults to `""`. | `string` |
| <a name="input_cluster_issuer_argo_helm_wait_backoff_limit"></a> [cluster\_issuer\_argo\_helm\_wait\_backoff\_limit](#input\_cluster\_issuer\_argo\_helm\_wait\_backoff\_limit) | Backoff limit for ArgoCD Application Helm release wait job. Defaults to `6`. | `number` |
| <a name="input_cluster_issuer_argo_helm_wait_kubectl_version"></a> [cluster\_issuer\_argo\_helm\_wait\_kubectl\_version](#input\_cluster\_issuer\_argo\_helm\_wait\_kubectl\_version) | Version of kubectl to use for ArgoCD Application wait job. Defaults to `1.32.3`. | `string` |
| <a name="input_cluster_issuer_argo_helm_wait_node_selector"></a> [cluster\_issuer\_argo\_helm\_wait\_node\_selector](#input\_cluster\_issuer\_argo\_helm\_wait\_node\_selector) | Node selector for ArgoCD Application Helm release wait job. Defaults to `{}`. | `map(string)` |
| <a name="input_cluster_issuer_argo_helm_wait_timeout"></a> [cluster\_issuer\_argo\_helm\_wait\_timeout](#input\_cluster\_issuer\_argo\_helm\_wait\_timeout) | Timeout for ArgoCD Application Helm release wait job. Defaults to `10m`. | `string` |
| <a name="input_cluster_issuer_argo_helm_wait_tolerations"></a> [cluster\_issuer\_argo\_helm\_wait\_tolerations](#input\_cluster\_issuer\_argo\_helm\_wait\_tolerations) | Tolerations for ArgoCD Application Helm release wait job. Defaults to `[]`. | `list(any)` |
| <a name="input_cluster_issuer_argo_info"></a> [cluster\_issuer\_argo\_info](#input\_cluster\_issuer\_argo\_info) | ArgoCD Application manifest info parameter. Defaults to `[{"name": "terraform", "value": "true"}]`. | `list(any)` |
| <a name="input_cluster_issuer_argo_kubernetes_manifest_computed_fields"></a> [cluster\_issuer\_argo\_kubernetes\_manifest\_computed\_fields](#input\_cluster\_issuer\_argo\_kubernetes\_manifest\_computed\_fields) | List of paths of fields to be handled as "computed". The user-configured value for the field will be overridden by any different value returned by the API after apply. Defaults to `["metadata.labels", "metadata.annotations", "metadata.finalizers"]`. | `list(string)` |
| <a name="input_cluster_issuer_argo_kubernetes_manifest_field_manager_force_conflicts"></a> [cluster\_issuer\_argo\_kubernetes\_manifest\_field\_manager\_force\_conflicts](#input\_cluster\_issuer\_argo\_kubernetes\_manifest\_field\_manager\_force\_conflicts) | Forcibly override any field manager conflicts when applying the kubernetes manifest resource. Defaults to `false`. | `bool` |
| <a name="input_cluster_issuer_argo_kubernetes_manifest_field_manager_name"></a> [cluster\_issuer\_argo\_kubernetes\_manifest\_field\_manager\_name](#input\_cluster\_issuer\_argo\_kubernetes\_manifest\_field\_manager\_name) | The name of the field manager to use when applying the Kubernetes manifest resource. Defaults to `Terraform`. | `string` |
| <a name="input_cluster_issuer_argo_kubernetes_manifest_wait_fields"></a> [cluster\_issuer\_argo\_kubernetes\_manifest\_wait\_fields](#input\_cluster\_issuer\_argo\_kubernetes\_manifest\_wait\_fields) | A map of fields and a corresponding regular expression with a pattern to wait for. The provider will wait until the field matches the regular expression. Use * for any value. Defaults to `{}`. | `map(string)` |
| <a name="input_cluster_issuer_argo_metadata"></a> [cluster\_issuer\_argo\_metadata](#input\_cluster\_issuer\_argo\_metadata) | ArgoCD Application metadata configuration. Override or create additional metadata parameters. Defaults to `{"finalizers": ["resources-finalizer.argocd.argoproj.io"]}`. | `any` |
| <a name="input_cluster_issuer_argo_name"></a> [cluster\_issuer\_argo\_name](#input\_cluster\_issuer\_argo\_name) | Name of the ArgoCD Application. Required if `argo_source_type` is set to `kustomize` or `directory`.  If `argo_source_type` is set to `helm`, ArgoCD Application name will equal `helm_release_name`. Defaults to `null`. | `string` |
| <a name="input_cluster_issuer_argo_namespace"></a> [cluster\_issuer\_argo\_namespace](#input\_cluster\_issuer\_argo\_namespace) | Namespace to deploy ArgoCD Application to. Defaults to `argo`. | `string` |
| <a name="input_cluster_issuer_argo_operation"></a> [cluster\_issuer\_argo\_operation](#input\_cluster\_issuer\_argo\_operation) | ArgoCD Application manifest operation parameter. Defaults to `{}`. | `any` |
| <a name="input_cluster_issuer_argo_project"></a> [cluster\_issuer\_argo\_project](#input\_cluster\_issuer\_argo\_project) | ArgoCD Application project. Defaults to `default`. | `string` |
| <a name="input_cluster_issuer_argo_source_path"></a> [cluster\_issuer\_argo\_source\_path](#input\_cluster\_issuer\_argo\_source\_path) | ArgoCD Application source path. Required if `argo_source_type` is set to `kustomize` or `directory`. Defaults to `null`. | `string` |
| <a name="input_cluster_issuer_argo_source_repo_url"></a> [cluster\_issuer\_argo\_source\_repo\_url](#input\_cluster\_issuer\_argo\_source\_repo\_url) | ArgoCD Application source repo URL. Required if `argo_source_type` is set to `kustomize` or `directory`. Defaults to `null`. | `string` |
| <a name="input_cluster_issuer_argo_source_target_revision"></a> [cluster\_issuer\_argo\_source\_target\_revision](#input\_cluster\_issuer\_argo\_source\_target\_revision) | ArgoCD Application source target revision. Required if `argo_source_type` is set to `kustomize` or `directory`. Defaults to `null`. | `string` |
| <a name="input_cluster_issuer_argo_source_type"></a> [cluster\_issuer\_argo\_source\_type](#input\_cluster\_issuer\_argo\_source\_type) | Source type for ArgoCD Application. Can be either `helm`, `kustomize`, or `directory`. Defaults to `helm`. | `string` |
...
| <a name="input_cluster_issuer_helm_chart_name"></a> [cluster\_issuer\_helm\_chart\_name](#input\_cluster\_issuer\_helm\_chart\_name) | Helm chart name to be installed. Required if `argo_source_type` is set to `helm`. Defaults to `null`. | `string` |
| <a name="input_cluster_issuer_helm_chart_version"></a> [cluster\_issuer\_helm\_chart\_version](#input\_cluster\_issuer\_helm\_chart\_version) | Version of the Helm chart. Required if `argo_source_type` is set to `helm`. Defaults to `null`. | `string` |
| <a name="input_cluster_issuer_helm_cleanup_on_fail"></a> [cluster\_issuer\_helm\_cleanup\_on\_fail](#input\_cluster\_issuer\_helm\_cleanup\_on\_fail) | Allow deletion of new resources created in this Helm upgrade when upgrade fails. Defaults to `false`. | `bool` |
| <a name="input_cluster_issuer_helm_create_namespace"></a> [cluster\_issuer\_helm\_create\_namespace](#input\_cluster\_issuer\_helm\_create\_namespace) | Create the Namespace if it does not yet exist. Defaults to `true`. | `bool` |
| <a name="input_cluster_issuer_helm_dependency_update"></a> [cluster\_issuer\_helm\_dependency\_update](#input\_cluster\_issuer\_helm\_dependency\_update) | Runs Helm dependency update before installing the chart. Defaults to `false`. | `bool` |
| <a name="input_cluster_issuer_helm_description"></a> [cluster\_issuer\_helm\_description](#input\_cluster\_issuer\_helm\_description) | Set Helm release description attribute (visible in the history). Defaults to `""`. | `string` |
| <a name="input_cluster_issuer_helm_devel"></a> [cluster\_issuer\_helm\_devel](#input\_cluster\_issuer\_helm\_devel) | Use Helm chart development versions, too. Equivalent to version '>0.0.0-0'. If version is set, this is ignored. Defaults to `false`. | `bool` |
| <a name="input_cluster_issuer_helm_disable_openapi_validation"></a> [cluster\_issuer\_helm\_disable\_openapi\_validation](#input\_cluster\_issuer\_helm\_disable\_openapi\_validation) | If set, the installation process will not validate rendered Helm templates against the Kubernetes OpenAPI Schema. Defaults to `false`. | `bool` |
| <a name="input_cluster_issuer_helm_disable_webhooks"></a> [cluster\_issuer\_helm\_disable\_webhooks](#input\_cluster\_issuer\_helm\_disable\_webhooks) | Prevent Helm chart hooks from running. Defaults to `false`. | `bool` |
| <a name="input_cluster_issuer_helm_enabled"></a> [cluster\_issuer\_helm\_enabled](#input\_cluster\_issuer\_helm\_enabled) | Set to false to prevent installation of the module via Helm release. Defaults to `true`. | `bool` |
| <a name="input_cluster_issuer_helm_force_update"></a> [cluster\_issuer\_helm\_force\_update](#input\_cluster\_issuer\_helm\_force\_update) | Force Helm resource update through delete/recreate if needed. Defaults to `false`. | `bool` |
| <a name="input_cluster_issuer_helm_keyring"></a> [cluster\_issuer\_helm\_keyring](#input\_cluster\_issuer\_helm\_keyring) | Location of public keys used for verification. Used only if `helm_package_verify` is `true`. Defaults to `~/.gnupg/pubring.gpg`. | `string` |
| <a name="input_cluster_issuer_helm_lint"></a> [cluster\_issuer\_helm\_lint](#input\_cluster\_issuer\_helm\_lint) | Run the Helm chart linter during the plan. Defaults to `false`. | `bool` |
| <a name="input_cluster_issuer_helm_package_verify"></a> [cluster\_issuer\_helm\_package\_verify](#input\_cluster\_issuer\_helm\_package\_verify) | Verify the package before installing it. Helm uses a provenance file to verify the integrity of the chart; this must be hosted alongside the chart. Defaults to `false`. | `bool` |
| <a name="input_cluster_issuer_helm_postrender"></a> [cluster\_issuer\_helm\_postrender](#input\_cluster\_issuer\_helm\_postrender) | Value block with a path to a binary file to run after Helm renders the manifest which can alter the manifest contents. Defaults to `{}`. | `map(any)` |
| <a name="input_cluster_issuer_helm_recreate_pods"></a> [cluster\_issuer\_helm\_recreate\_pods](#input\_cluster\_issuer\_helm\_recreate\_pods) | Perform pods restart during Helm upgrade/rollback. Defaults to `false`. | `bool` |
| <a name="input_cluster_issuer_helm_release_max_history"></a> [cluster\_issuer\_helm\_release\_max\_history](#input\_cluster\_issuer\_helm\_release\_max\_history) | Maximum number of release versions stored per release. Defaults to `0`. | `number` |
| <a name="input_cluster_issuer_helm_release_name"></a> [cluster\_issuer\_helm\_release\_name](#input\_cluster\_issuer\_helm\_release\_name) | Helm release name. Required if `argo_source_type` is set to `helm`. Defaults to `null`. | `string` |
| <a name="input_cluster_issuer_helm_render_subchart_notes"></a> [cluster\_issuer\_helm\_render\_subchart\_notes](#input\_cluster\_issuer\_helm\_render\_subchart\_notes) | If set, render Helm subchart notes along with the parent. Defaults to `true`. | `bool` |
| <a name="input_cluster_issuer_helm_replace"></a> [cluster\_issuer\_helm\_replace](#input\_cluster\_issuer\_helm\_replace) | Re-use the given name of Helm release, only if that name is a deleted release which remains in the history. This is unsafe in production. Defaults to `false`. | `bool` |
| <a name="input_cluster_issuer_helm_repo_ca_file"></a> [cluster\_issuer\_helm\_repo\_ca\_file](#input\_cluster\_issuer\_helm\_repo\_ca\_file) | Helm repositories CA cert file. Defaults to `""`. | `string` |
| <a name="input_cluster_issuer_helm_repo_cert_file"></a> [cluster\_issuer\_helm\_repo\_cert\_file](#input\_cluster\_issuer\_helm\_repo\_cert\_file) | Helm repositories cert file. Defaults to `""`. | `string` |
| <a name="input_cluster_issuer_helm_repo_key_file"></a> [cluster\_issuer\_helm\_repo\_key\_file](#input\_cluster\_issuer\_helm\_repo\_key\_file) | Helm repositories cert key file. Defaults to `""`. | `string` |
| <a name="input_cluster_issuer_helm_repo_password"></a> [cluster\_issuer\_helm\_repo\_password](#input\_cluster\_issuer\_helm\_repo\_password) | Password for HTTP basic authentication against the Helm repository. Defaults to `""`. | `string` |
| <a name="input_cluster_issuer_helm_repo_url"></a> [cluster\_issuer\_helm\_repo\_url](#input\_cluster\_issuer\_helm\_repo\_url) | Helm repository. Required if `argo_source_type` is set to `helm`. Defaults to `null`. | `string` |
| <a name="input_cluster_issuer_helm_repo_username"></a> [cluster\_issuer\_helm\_repo\_username](#input\_cluster\_issuer\_helm\_repo\_username) | Username for HTTP basic authentication against the Helm repository. Defaults to `""`. | `string` |
| <a name="input_cluster_issuer_helm_reset_values"></a> [cluster\_issuer\_helm\_reset\_values](#input\_cluster\_issuer\_helm\_reset\_values) | When upgrading, reset the values to the ones built into the Helm chart. Defaults to `false`. | `bool` |
| <a name="input_cluster_issuer_helm_reuse_values"></a> [cluster\_issuer\_helm\_reuse\_values](#input\_cluster\_issuer\_helm\_reuse\_values) | When upgrading, reuse the last Helm release's values and merge in any overrides. If `helm_reset_values` is specified, this is ignored. Defaults to `false`. | `bool` |
| <a name="input_cluster_issuer_helm_set_sensitive"></a> [cluster\_issuer\_helm\_set\_sensitive](#input\_cluster\_issuer\_helm\_set\_sensitive) | Value block with custom sensitive values to be merged with the values yaml that won't be exposed in the plan's diff. Defaults to `{}`. | `map(any)` |
| <a name="input_cluster_issuer_helm_skip_cluster_issuer"></a> [cluster\_issuer\_helm\_skip\_cluster\_issuer](#input\_cluster\_issuer\_helm\_skip\_cluster\_issuer) | If set, no cluster\_issuer will be installed before Helm release. Defaults to `false`. | `bool` |
| <a name="input_cluster_issuer_helm_timeout"></a> [cluster\_issuer\_helm\_timeout](#input\_cluster\_issuer\_helm\_timeout) | Time in seconds to wait for any individual Kubernetes operation (like Jobs for hooks). Defaults to `300`. | `number` |
| <a name="input_cluster_issuer_helm_wait"></a> [cluster\_issuer\_helm\_wait](#input\_cluster\_issuer\_helm\_wait) | Will wait until all Helm release resources are in a ready state before marking the release as successful. It will wait for as long as timeout. Defaults to `false`. | `bool` |
| <a name="input_cluster_issuer_helm_wait_for_jobs"></a> [cluster\_issuer\_helm\_wait\_for\_jobs](#input\_cluster\_issuer\_helm\_wait\_for\_jobs) | If wait is enabled, will wait until all Helm Jobs have been completed before marking the release as successful. It will wait for as long as timeout. Defaults to `false`. | `bool` |
