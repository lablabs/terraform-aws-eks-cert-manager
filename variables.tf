...
variable "namespace" {
  type        = string
  default     = "kube-system"
  description = "The K8s namespace in which the cert manager will be installed"
}
...
variable "irsa_role_name_prefix" {
  ...
  description = "The IRSA role name prefix for cert-manager"
}

variable "irsa_assume_role_enabled" {
  type        = bool
  default     = false
  description = "Whether IRSA is allowed to assume role defined by irsa_assume_role_arn. Useful for hosted zones in another AWS account."
}
...
variable "manifest_target_revision" {
  type        = string
  default     = "v2.0.0" #FIXME: update revision before release
